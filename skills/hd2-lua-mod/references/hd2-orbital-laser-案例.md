...
成品发出去之后收到的反馈,基本都不是"算法错了",而是**分发和环境**问题 —— 这一节可能比前面更值得抄。

| 反馈 | 真因 | 修法 |
|---|---|---|
| **① 完全不生效** | 大多是没装/没开 Loader、或没重启游戏;少数是构建漂移 | 新增 `STATUS.txt`,第一行直接给结论 |
| **② 一开始生效、过一会儿失效** | **同一张表在内存里有多份副本**(普查里同类型哈希的块几十个),换图还会再加载新的;旧版**只打了第一个命中的记录** | 全部副本都打 + 每 20 秒"观察窗口"轻扫 + 每 10 分钟全量兜底 |
| **③ 无法导入 mod manager** | `manifest.json` 的 `Name` 里有一个 **ASCII 冒号**(Windows 非法文件名字符),而管理器拿 `Name` 当文件夹名 | `Name` 改成纯 ASCII 且无非法字符;打包脚本加自检 |
| **④ 与另一个 mod 冲突** | 代码/数据层面无冲突(不同的表、不同的单例键、`ffi.cdef` 逐字节一致)。真凶是**手动安装时两个 zip 里同名的 `Addon/9ba626afa44a3aa3.patch_0` 互相覆盖**,加上两个全内存扫描器叠加的帧率开销 | 说明里写明必须用管理器装;并降低扫描代价 |

### 7.1 ② 是真的设计缺陷

"打补丁"被当成了**一次性动作**。可这类表是会被游戏**重新加载**的 ——
内存里本来就躺着几十份副本,你自己挑中的那份还带着补丁,游戏却已经在用另一份了。
**复查逻辑于是完全正确地报告"一切正常",而玩家看到的一切照旧。**

教训:**任何"打一次就完"的运行时补丁,都要问一句"这份数据会不会被重新加载、有没有别的副本"。**
维护的代价可以很低 —— 记下每个已知区块附近的窗口(±32 KB 合并),之后只扫这几 MB。

### 7.2 复查代码里那个静默 bug

改成多副本之后,复查要用"当前字节"判定,于是把刚读回来的 400 字节塞回记录结构。
但记录结构里有个"记录在区块内的偏移"字段,而刚读回来那 400 字节是**从记录本身开始**的 ——
不把它归 1,后面按 `off + 字段偏移` 取值就会**越界读到 nil**,函数静默返回 "unreadable",
补丁再也不会被重打。离线仿真(`TEST 3`:把补丁擦掉,要求它自己重新打上)一把抓住。

### 7.3 发布物的命名也是代码

一个冒号,整个 mod 导不进。**`manifest` 里的 `Name` 是给操作系统当文件名用的**,不是给人看的标签。
所以要在打包脚本里自检,而不是指望发布前有人想起来。

### 7.4 反馈要能变成信息

用户的报告是"不生效"。要把这句话变成可执行的东西,只能靠**预置的产物**:
`STATUS.txt`(给用户)+ `lld_census_roundN.txt` / `strat_blocks_roundN.hex` / 日志(给自己)。
"让用户去找日志"是收不到反馈的;"让用户贴一个 8 行的文件"才收得到。
...
| `analysis/test_orbital_laser_free.py` | 10 组功能测试 + 12 组变异测试(含"两份副本都要打") |
| `analysis/check_manifest.py` | 安装包 `Name` 的 ASCII / Windows 文件名字符自检 |
