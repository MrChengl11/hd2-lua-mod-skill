...
### 6.48 飞船大厅缺表场景的重试治理与多表就绪守卫

**惨痛教训**: 部分表格（如 `EagleComponentData`）在飞船（大厅）阶段并未加载进内存，只有进入任务后才常驻。
如果在治理周期性扫描时，盲目将 `maintain_seconds` 设为 0，会导致 Mod 彻底放弃搜索飞鹰组件表！
而战备表（`StratagemSettings`）只决定“能不能呼叫飞鹰”，真正的“投弹类型、弹数、投弹间隔”全由 `EagleComponentData` 决定。
飞鹰表未打上补丁时，打击实体保持为原版空对空导弹（AIM-9），实机表现为**飞机呼啸飞过，却没有任何炸弹落下**！

**规则: 双表依赖型 Mod 必须实行「首轮直搜 + 入局捕获即永久休眠」与「双表就绪降频守卫」:**
1. **战备打完立即初搜**: 战备表一就绪，立即无延迟启动第 1 轮飞鹰表扫描，绝不先等几十秒；
2. **大厅缺表温和重试**: 若首轮未搜到（在大厅），设置 10 秒适度间隔退避；一旦进入任务对局搜到并打上补丁，立即置位 `state.eagle_found = true`，**从此永久退出扫描，绝不重复调用 VirtualQuery**；
3. **双表就绪降频守卫**: `my_update` 步长放宽至 60 帧的前提必须是**所有依赖表均已完成（`state.phase == 'patched' and state.eagle_found`）**。在飞鹰表仍在搜索期间保持 `SCAN_EVERY = 2` 步长，保证进图后 1~2 秒内迅速定位打完，杜绝提前降频导致搜索严重拖慢。
