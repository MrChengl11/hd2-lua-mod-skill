...
| [飞鹰地毯式空袭](skills/hd2-lua-mod/references/hd2-carpet-bomb-案例.md) | 把**从未发布、且投放载具已被删掉**的战备搬回游戏。7 轮实机迭代,踩满:仿真器 Lua 版本不对、字段其实是 `byte` 且位打包、**UI 列表的闸门不在数据里**(只能借壳)、借壳会继承原战备机制、卡面数值是标定量、枚举下标被版本回收、payload 数组的语义。**想知道怎么让隐藏内容被玩家看见,先看这篇** |
